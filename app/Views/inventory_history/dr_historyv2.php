...
<div id="editModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
  <div class="bg-white rounded-2xl shadow-lg w-full max-w-lg p-6">
    <h2 class="text-xl font-bold mb-4">Edit Delivery Record</h2>
    ...
    <form id="editForm" class="space-y-4">
      ...
      <div>
        ...
        <input type="text" id="modal_dr_number" class="mt-1 w-full border rounded-lg px-3 py-2">
      </div>

      <div>
        <label class="block text-sm font-medium">Name</label>
        <input type="text" id="modal_name" class="mt-1 w-full border rounded-lg px-3 py-2">
      </div>

      <div>
        <label class="block text-sm font-medium">Address</label>
        <input type="text" id="modal_address" class="mt-1 w-full border rounded-lg px-3 py-2">
      </div>

      <div>
        <label class="block text-sm font-medium">Ref PO Number</label>
        <input type="text" id="modal_ref_po_number" class="mt-1 w-full border rounded-lg px-3 py-2">
      </div>

      <div class="flex justify-end space-x-2 mt-4">
        <button type="button" onclick="closeModal()" class="px-4 py-2 rounded-lg bg-gray-300 hover:bg-gray-400">Cancel</button>
        <button type="submit" class="px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700">Confirm</button>
      </div>
    </form>
  </div>
</div>
...
<script>
        ...
        result.data.forEach((recordv2, index) => {
            ...
            row.innerHTML = `
                <td class=" bg-[#FFFFFF80] px-4 py-2 border-b font-bold">
                    ...
                    <button onclick="editModal('${recordv2.id}','${recordv2.dr_number}','${(recordv2.name)?recordv2.name:blank}','${(recordv2.address)?recordv2.address:blank}','${(recordv2.ref_po_number)?recordv2.ref_po_number:blank}')" class="text-yellow-600 hover:underline">Edit</button>
                </td>
                <td class=" bg-[#FFFFFF80] px-4 py-2 border-b font-bold">
                    ${recordv2.dr_number}
                </td>
    ...
    function editModal(id, dr_number, name, address, ref_po_number) {
        // Fill modal inputs
        document.getElementById("modal_id").value = (id)?id:'';
        document.getElementById("modal_dr_number").value = (dr_number)?dr_number:'';
        document.getElementById("modal_name").value = (name)?name:'';
        document.getElementById("modal_address").value = (address)?address:'';
        document.getElementById("modal_ref_po_number").value = (ref_po_number)?ref_po_number:'';
    ...
    document.getElementById("editForm").addEventListener("submit", async function(e) {
        ...
        const data = {
            ...
            dr_number: document.getElementById("modal_dr_number").value,
            name: document.getElementById("modal_name").value,
            address: document.getElementById("modal_address").value,
            ref_po_number: document.getElementById("modal_ref_po_number").value
        };

        try {
            const response = await fetch(`${base_url}inventory_history_group/updateGroup/`+data.id, {
            method: "POST",
            headers: {
            ...
            location.reload();
            // refresh table or update UI here
            } else {
